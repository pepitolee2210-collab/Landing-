...
const NAV_LINKS = [
  ...
  { href: '/#testimonios', label: 'Casos reales' },
  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-500 ${
        scrolled
          ? 'border-b border-[var(--color-line-soft)] backdrop-blur-md'
          : 'border-b border-transparent'
      }`}
      style={{
        background: scrolled
          ? 'color-mix(in oklab, var(--color-canvas) 86%, transparent)'
          : 'transparent',
      }}
    >
      <div className="container-editorial flex items-center justify-between py-4">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="link-underline text-sm text-[var(--color-ink-2)] hover:text-[var(--color-ink)] transition-colors"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-3">
          <a
            href={SITE.appUrl}
            className="hidden sm:inline-flex text-xs font-medium text-[var(--color-ink-3)] hover:text-[var(--color-ink)] transition-colors px-2"
          >
            Ya soy cliente →
          </a>
          <a
